...
def extrace_object_demo():
    ...
    while True:
        ...
        frame = color_frame
        blur_frame = cv.GaussianBlur(color_frame, (7, 7), 0)
        # cv.imshow("blurred", frame)
        mask = cv.inRange(blur_frame, lowerb=np.array([0, 0, cv.getTrackbarPos('R', 'frame')]), upperb=np.array([cv.getTrackbarPos('B', 'frame'), cv.getTrackbarPos('G', 'frame'), 255]))
        ...
        for i, contour in enumerate(contours):
            ...
            if  contour_area > 0.8 * w * h and contour_area > 1000 and w > h:
